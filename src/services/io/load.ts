...
async function parseLoadedData(data: string[], mapVersion: string | null): Promise<void> {
  try {
...

    {
      ...
      if (options.winds) window.loadedMapWinds = [...options.winds];
      ...
      const areaModeInput = document.getElementById("areaModeInput") as HTMLInputElement | null;
      if (areaModeInput) areaModeInput.value = useSphericalArea ? "spherical" : "flat";
    }
    ...
    resourcesLayer = viewbox.select<SVGGElement>("#resources");
    climate = viewbox.select<SVGGElement>("#climate");
    ...
    if (!resourcesLayer.size()) {
      resourcesLayer = viewbox
        .insert("g", "#cells")
        .attr("id", "resources")
        .style("display", "none") as unknown as typeof resourcesLayer;
    }
    if (!climate.size()) {
      climate = viewbox
        .insert("g", "#population")
        .attr("id", "climate")
        .style("display", "none") as unknown as typeof climate;
    }
    if (!goods.size()) {
      goods = viewbox.insert("g", "#markets").attr("id", "goods").style("display", "none") as unknown as typeof goods;
    }
    if (!markets.size()) {
      markets = viewbox.insert("g", "#emblems").attr("id", "markets") as unknown as typeof markets;
    }
    if (!tradeAnimation.size()) {
      tradeAnimation = viewbox.insert("g", "#ruler").attr("id", "tradeAnimation") as unknown as typeof tradeAnimation;
    }
    conts = viewbox.select<SVGGElement>("#conts");
    if (!conts.size()) {
      conts = viewbox.insert("g", "#cults").attr("id", "conts") as unknown as typeof conts;
    }
    ...
    pack.cells.resourceSecondary = data[43]
      ? Uint8Array.from(data[43].split(","), Number)
      : new Uint8Array(pack.cells.i.length);
    pack.cells.resourceRichness = data[44]
      ? Uint8Array.from(data[44].split(","), Number)
      : new Uint8Array(pack.cells.i.length);
    pack.cells.good = data[45] ? Uint16Array.from(data[45].split(","), Number) : new Uint16Array(pack.cells.i.length);
    ...
    if (data[50]) {
      ...
      if (goodIconsDefs) goodIconsDefs.insertAdjacentHTML("beforeend", data[50]);
    }
