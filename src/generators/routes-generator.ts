...
class RoutesModule {
  ...
  private createCostEvaluator({
    ...
    if (isWater) return this.getWaterPathCost.bind(this);

    const connections = this.connections;
    return function getLandPathCostCustom(current: number, next: number) {
      if (pack.cells.h[next] < 20) return Infinity; // ignore water cells
      ...
      return distanceCost * habitabilityModifier * heightModifier * connectionModifier * burgModifier * hostileMod;
    };
  }
  ...
  private generateMainRoads() {
    ...
    for (const [key, featureCapitals] of Object.entries(capitalsByFeature)) {
      ...
      urquhartEdges.forEach(([fromId, toId]) => {
        ...
        const start = burgA.cell!;
        const exit = burgB.cell!;
